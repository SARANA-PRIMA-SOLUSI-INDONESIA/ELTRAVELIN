...
import { wibStartOfDay, wibEndOfDay } from "@/lib/on-demand-schedules";
...
export async function GET(request: Request) {
  ...
  const schedules = await prisma.schedule.findMany({ where: { isDeleted: false, departureTime: { gte: wibStartOfDay(from), lte: wibEndOfDay(to) }, template: { isActive: true } }, orderBy: { departureTime: "asc" }, include: { route: true, operatingTrip: { include: { driver: true } }, bookings: { where: { status: { not: "CANCELLED" } }, select: { id: true } } } });
