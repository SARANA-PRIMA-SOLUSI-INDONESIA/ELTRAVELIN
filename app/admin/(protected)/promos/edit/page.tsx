...
import { getPromoById, updatePromo } from "@/app/actions/admin-promo";
...
function EditPromoForm() {
  ...
  useEffect(() => {
    ...
    getPromoById(promoId)
      .then((p: any) => {
        if (!p) {
          showError({ title: "Gagal", text: "Promo tidak ditemukan." });
        ...
        setMaxDiscount(p.maxDiscount ?? undefined);
        ...
        setShowOnCheckout(p.showOnCheckout);
        setFetching(false);
      })
      .catch(async (err) => {
        await showError({ title: "Gagal", text: "Gagal memuat data promo: " + err.message });
        router.push("/admin/promos");
      });
  }, [promoId, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await showError({ title: "Gagal", text: "Gagal memperbarui promo: " + (error as Error).message });
    } finally {
      setLoading(false);
    }
  };
  ...
  if (fetching) {
    return (
      <div className="flex items-center justify-center p-24">
        <p className="text-sm text-foreground/40 font-medium">Memuat data promo...</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-10 max-w-2xl">
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col">
          <h1 className="text-3xl font-display font-bold text-navy-deep">Edit Promo</h1>
          <p className="text-sm text-foreground/60">Perbarui detail kode promo.</p>
        </div>
      </div>
...
export default function EditPromo() {
  return (
    <Suspense fallback={<div className="p-24 text-center">Memuat Editor Promo...</div>}>
      <EditPromoForm />
    </Suspense>
  );
}
